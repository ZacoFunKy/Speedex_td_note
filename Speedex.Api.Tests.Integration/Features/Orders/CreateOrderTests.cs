...
public class CreateOrderTests : IClassFixture<CustomWebApplicationFactory<Program>>
{
    ...
    [Fact]
    public async Task CreateOrder_With_Multiple_Products_Should_ReturnCreatedStatusCode_And_FindCreatedOrder_And_Have_Correct_TotalPrice_USD_To_EUR()
    {
        // Arange 
        var httpClient = _factory.CreateClient();
        ...
        var order = getOrderResponse.Items.First();
        Assert.NotNull(order);
        Assert.Equal( (decimal)58.80 , order.TotalPriceEur);
    }
